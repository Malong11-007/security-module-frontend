  ...
  let CompletDate = year+'-'+month+'-'+date;
	...
	const OnInsert = data => {
		...
		.then(function(response) {
			...
			{
			  swal("Submited", "Your New RolesForms's Body Is Successfully Inserted", "success");
			}
			//
			  // swal (validate)
			  else if(response.status === 403)
			  {
			  swal("Not Submited", "Your New RolesForms's Body Is Missing Or Validate", "warning");
			  }
			  //
			  // swal (error)
			  else if(response.status === 400)
			  {
				  swal("Not Submited", "Your New RolesForms's Body Is not Submited Successfully ", "error");
			  }
			  //
		})
	...
	const OnUpdate = data => {
		...
		.then(function(response) {
			...
			if(response.status === 200)
			{
			  swal("Submited", "Your New RolesForm's Body Is Successfully Updated", "success");
			}
			//
			// swal (validate)
			 else if(response.status === 403)
			 {
			   swal("Not Submited", "Your New RolesForm's Body Is Missing Or Validate", "warning");
			 }
			 //
			 // swal (error)
			 else if(response.status === 400)
			 {
				 swal("Not Submited", "Your New RolesForm's Body Is not Updated Successfully ", "error");
			 }
			 //
		})
		.catch(function(error) {
